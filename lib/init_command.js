...
module.exports = class Command {
  constructor (options) {
    options = options || {}
    this.name = options.name || 'klg-init'
    this.configName = options.configName || 'klg-init-config'
  ...
  * run (cwd, args) {
   ...
   *
   * @param {Object} mapping - boilerplate config mapping, `{ simple: { "name": "simple", "package": "klg-boilerplate-simple", "description": "Simple klg app boilerplate" } }`
  ...
  * askForVariable (targetDir, templateDir) {
    ...
    try {
      ...
      if (questions.name && !questions.name.default) {
        questions.name.default = path.basename(targetDir).replace(/^klg-/, '')
      }
    } catch (err) {
  ...
  * processFiles (targetDir, templateDir) {
  ...
  getParser () {
    return yargs
      .usage('init klg project from boilerplate.\nUsage: $0 [dir] --type=simple')
  ...
  * getTemplateDir () {
    let templateDir
    // when use `klg-init --template=PATH`
  ...
  /**
   * 模板类型配置
   * @returns {{boilerplate: {module: {package: string, description: string}, framework: {package: string, description: string}}}}
   */
  * fetchBoilerplateMapping () {
    return {
      'module': {
        'package': 'klg-boilerplate-module',
        'description': 'klg npm module'
      }
      // 'framework': {
      //   'package': 'klg-boilerplate-framework',
      //   'description': 'klg typescript framework boilerplate'
      // },
      // 'framework-ts': {
      //   'package': 'klg-boilerplate-framework-ts',
      //   'description': 'klg typescript framework boilerplate'
      // }
    }
  }
  ...
  * downloadBoilerplate (pkgName) {
    ...
    const saveDir = path.join(os.tmpdir(), 'klg-init-boilerplate')
